...
| **سقف هزینه ماهانه (Budget Quota)** | هر Tenant | تعیین سقف هزینه ماهانه برای هر Tenant بر اساس ترکیب مصرف مدل، ذخیره‌سازی و محاسبات؛ در صورت عبور از ۸۰٪ هشدار و در صورت عبور از ۱۰۰٪ محدودسازی درخواست‌های با اولویت پایین (طبق `Multi_Tenancy_Architecture` بخش ۹) |
