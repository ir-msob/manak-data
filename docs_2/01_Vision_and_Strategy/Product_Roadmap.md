...
# سند نقشه راه و فازبندی محصول (Product Roadmap)
...
این سند مسیر تکامل محصول فراتر از MVP (نسخه اولیه، تعریف‌شده در سند `High_Level_PRD`) را در قالب فازهای توسعه مشخص می‌کند. ترتیب فازها بر اساس اولویت ارزش کسب‌وکاری و پیش‌نیاز فنی است، نه لزوماً بازه زمانی ثابت.
...
پیاده‌سازی چرخه کامل Connect–Understand–Reason–Act–Learn در کوچک‌ترین حالت قابل استفاده. این فاز دقیقاً معادل محدوده MVP تعریف‌شده در سند `High_Level_PRD` است و شامل موارد زیر می‌شود:
...
| نام سند | ارتباط |
| :--- | :--- |
| `High_Level_PRD` | مرجع دقیق محدوده MVP که فاز ۱ بر اساس آن تعریف شده |
| `Naming_And_Terminology_Glossary` | مرجع واژه‌نامه و استاندارد نام‌گذاری مورد استفاده در این سند |
| `Architecture_Overview` | مرجع Componentها و سرویس‌های Governance که در فازهای ۰، ۲ و ۴ توسعه می‌یابند |
