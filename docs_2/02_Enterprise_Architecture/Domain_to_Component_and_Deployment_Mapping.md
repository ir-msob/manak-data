...
| دامنه | Component (از `Component_Map_And_Responsibilities`) | واحد استقرار پیشنهادی | نوع | دلیل |
| :--- | :--- | :--- | :--- | :--- |
...
| `Memory_and_Experience_Domain_Architecture` | مرجع مالکیت Shared Agent Context Store که در `session-memory-service` تعبیه شده است |
...
**مسئولیت به‌روزرسانی:** تیم معماری به همراه تیم DevOps مسئول بازبینی این نگاشت پیش از شروع هر فاز جدید از `Product_Roadmap` هستند، چون تفکیک/ادغام Serviceها ممکن است با رشد بار واقعی تغییر کند.
