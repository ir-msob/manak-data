...
| لایه | نگاشت به معماری (`Architecture_Overview`) |
| :--- | :--- |
...
- **پیکربندی مبتنی بر محیط (Environment-specific Configuration):** هر محیط دارای مجموعه‌ای از متغیرهای پیکربندی مجزا است که از طریق سرویس مدیریت پیکربندی (Configuration Management Service — که در سند `Architecture_Overview` تعریف شده) مدیریت می‌شود.
...
| سند | ارتباط |
| :--- | :--- |
| `Architecture_Overview` | مرجع لایه‌های معماری که در بخش ۵ همین سند به توپولوژی استقرار نگاشت شده‌اند |
...
| `Infrastructure_and_Operations_Domain.md` | مرجع پیاده‌سازی زیرساختی Resource Quota و مدیریت Cluster که با بخش ۶ این سند هم‌راستا است |
| `DevOps_and_CICD_Pipeline` | مرجع خط لوله CI/CD که در بخش ۹ به آن اشاره شده است |
