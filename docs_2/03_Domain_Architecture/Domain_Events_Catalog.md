...
| **قابلیت ردیابی کنشگر (Actor Traceability)** | هر رویداد مرتبط با اقدام کاربر یا سرویس باید هویت کنشگر آغازکننده را حمل کند تا ممیزی کامل در مسیرهای رویدادمحور امکان‌پذیر باشد (طبق `Event_and_Webhook_Domain.md` بخش ۴.۲.۱). |
...
| EVT-035 | **PDPFallbackActivated** | v1.0 | حاکمیت و انطباق | مشاهده‌پذیری، عملیات | یک PEP به‌دلیل عدم دسترسی به PDP، به کش محلی تصمیمات سوئیچ کرد | رفتار Fallback موقت PDP (طبق `Governance_and_Compliance_Domain.md` بخش ۴.۲.۱) فعال شد. | `pep_id`, `service_name`, `operation_risk_level`, `activated_at` | غیرهمزمان |
...
| **Schema Registry** | نگهداری و نسخه‌بندی ساختار رویدادها (میزبانی‌شده در `metadata-lineage-service`) | `Metadata_and_Lineage_Domain.md` |
| **Acknowledgment Mechanism** | مکانیزم تأیید دریافت، Timeout و Retry برای رویدادهای حیاتی (پیاده‌سازی‌شده در `Event_and_Webhook_Domain.md`) | `Event_and_Webhook_Domain.md` بخش ۴.۳ |
...
| نام سند | نوع ارتباط |
| :--- | :--- |
| `Domain_Landscape` | مرجع دامنه‌های اصلی که رویدادها در آن‌ها تولید یا مصرف می‌شوند |
...
| `Architecture_Overview` | مرجع معماری Event‑Driven که زیرساخت رویدادها بر اساس آن طراحی شده است |
...
| `Event_and_Webhook_Domain.md` | مرجع ساختار استاندارد Envelope (بخش ۴.۲.۱)، مکانیزم Acknowledgment (بخش ۴.۳) و الزام Actor Traceability که در بخش ۴ و ۵ این سند به آن ارجاع داده شده است |
| `Memory_and_Experience_Domain.md` | مرجع Shared Agent Context Store که رویدادهای `SharedContextUpdated`/`SharedContextConflict` (بخش ۶.۳) از آن تولید می‌شوند |
