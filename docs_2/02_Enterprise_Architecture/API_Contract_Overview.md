...
### ۴.۱. قرارداد نسخه‌گذاری گراف دانش (Knowledge Graph Versioning Contract)

**مصرف‌کننده:** Knowledge & Context Engine  
**ارائه‌دهنده:** Knowledge Management Domain (Graph Query Service)

**الزامات:**

- هر درخواست پرس‌وجوی گراف دانش باید شامل هدر `X-Graph-Version` با مقدار نسخه مورد انتظار باشد (مثلاً `2.1.0`).
- اگر هدر ارسال نشود، Graph Query Service باید از آخرین نسخه پایدار (Stable) استفاده کند و در Log هشدار ثبت کند.
- اگر نسخه درخواستی پشتیبانی نشود، Graph Query Service باید پاسخ با کد وضعیت `409 Conflict` و بدنه خطای زیر بازگرداند:
  ...
  "errors": [
    {
      ...
      "message": "نسخه گراف دانش درخواستی پشتیبانی نمی‌شود.",
      "details": {
        "requested_version": "2.1.0",
        "supported_versions": ["2.0.0", "1.5.0"],
        "deprecated_versions": ["1.5.0"],
...
| **Authorization** | string | بله | توکن JWT کاربر اصلی در قالب `Bearer <token>` که در تمام فراخوانی‌های داخلی نیز حفظ و منتقل می‌شود. |
...
```json
{
  ...
  "data": {
    ...
    "compensation": {
      "status": "AVAILABLE" | "NOT_SUPPORTED" | "FAILED",
      "action": "tool_name.compensate",
...
برای تمام پاسخ‌های مربوط به اجرای Tool، فیلد `compensation` به‌صورت اختیاری در بخش `data` درج می‌شود:

| فیلد | نوع | توضیح |
| :--- | :--- | :--- |
| `compensation.status` | string | وضعیت جبران: `AVAILABLE` (قابل اجرا), `NOT_SUPPORTED` (پشتیبانی نمی‌شود), `FAILED` (شکست در جبران) |
| `compensation.action` | string | نام عملیات جبرانی (در صورت وجود) |
| `compensation.requires_manual` | boolean | آیا عملیات جبرانی نیاز به مداخله دستی دارد؟ |
...
- تمام سرویس‌های داخلی موظف به پذیرش و ارسال هدر `Authorization` حاوی توکن JWT کاربر اصلی در تمام درخواست‌های HTTP هستند.
- در ارتباطات مبتنی بر رویداد (Message Broker)، هویت کاربر در Metadata رویداد (فیلد `user_id` و در صورت نیاز `user_token` برای اعتبارسنجی مجدد) منتقل می‌شود.
- سرویس‌های داخلی می‌توانند توکن JWT را اعتبارسنجی مجدد کنند (با استفاده از JWKS) یا به کش مرکزی اعتبارسنجی متکی باشند.
- در صورت انقضای توکن در حین پردازش‌های طولانی، سرویس Orchestrator موظف به دریافت توکن جدید از طریق Refresh Token (در صورت وجود) یا لغو درخواست است.
...
| نام سند | نوع ارتباط |
| :--- | :--- |
| `Architecture_Overview` | مرجع معماری Event-Driven که الگوهای ارتباطی بخش ۳ بر پایه آن استوار است. همچنین مرجع سرویس‌های مشترک (بخش ۶) که پیاده‌سازی امنیت، احراز هویت و Rate Limiting را مشخص می‌کنند. |
