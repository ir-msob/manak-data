...
> **توجه استقرار:** این موتور به‌عنوان یک مفهوم منطقی واحد طراحی شده، اما در عمل به دو زیرساخت مجزا تفکیک می‌شود: **`session-memory`** (با استفاده از Redis/Cache برای تأخیر بسیار کم) و **`long-term-memory`** (با استفاده از پایگاه داده رابطه‌ای و برداری برای جستجوی معنایی و ماندگاری بالا). جزئیات کامل در سند `Domain_to_Component_and_Deployment_Mapping` و `Memory_&_Experience_Domain_Architecture` آمده است.
...
| `High_Level_PRD` | نیازمندی‌های عملکردی و چرخه AI Loop که معماری برای پشتیبانی از آن‌ها طراحی شده است |
...
| `Component_Map_And_Responsibilities` | نقشه کامپوننت‌ها و مرز مسئولیت هر Component |
