...
| سند | ارتباط |
| :--- | :--- |
| `Architecture_Overview` | مرجع اصول معماری (Event-Driven، Zero Trust) که الگوهای همکاری این سند بر پایه آن‌هاست |
...
| `Security_and_Privacy_Domain.md` | مرجع پیاده‌سازی حاکمیت، Audit و کنترل هزینه اشاره‌شده در بخش ۱۰ این سند |
| `Memory_and_Experience_Domain.md` | مرجع مالکیت پیاده‌سازی، مدل داده و مدل همزمانی Shared Agent Context Store (بخش ۴.۱.۱ آن سند) که مفهوم منطقی بخش ۷ همین سند را پیاده‌سازی می‌کند |
